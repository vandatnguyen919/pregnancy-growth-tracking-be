...
public class PregnancyController {
    ...
    public Result getPregnanciesByUserId(@PathVariable Long userId, Pageable pageable) {
        Page<Pregnancy> pregnancyPage = pregnancyService.findByUserId(userId, pageable);
        Page<PregnancyDto> pregnancyDtoPage = pregnancyPage.map(this.pregnancyToDtoConverter::convert);
        return new Result(true, StatusCode.SUCCESS, "Find By User Success", pregnancyDtoPage);
    }

    @PostMapping
    public Result addPregnancy(@Valid @RequestBody PregnancyDto newPregnancyDto) {
    ...
    public Result updatePregnancy(@PathVariable Long pregnancyId, @Valid @RequestBody PregnancyDto pregnancyDto) {
